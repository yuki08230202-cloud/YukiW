...
|---|---|---|
| 電話番号(SMS)認証のみ | Apple/Google/メール認証、匿名利用 | BeReal 同様に電話番号のみとし、1 人 1 アカウントの実在性を担保。SMS コストは増えるが、複垢による不正(ストリーク偽装・嫌がらせ後の作り直し)の抑止と、将来の連絡先同期による友達推薦(v1.0)への布石として投資価値がある |
...
| コア体験 | **起床達成率** | 起床通知を受けたユーザーのうち制限時間内に投稿した割合 | 50%以上(猶予 3 分の緊迫感設計とのトレードオフを許容) |
| コア体験 | **平均連続達成日数** | アクティブユーザーの現在ストリークの平均 | 4 日以上 |
| コア体験 | **通知→投稿までの時間(中央値)** | 起床通知送達から投稿完了まで | 90 秒以内(猶予 3 分に対し) |
...
- **新規ユーザーとして**、電話番号と SMS コードだけで 1 分以内に登録を終えたい。**なぜなら**、寝る前や思い立った瞬間の熱が冷めないうちに始めたいし、パスワードを考えたくないから。
...
**仕様要点:**
- 認証方式:**電話番号+SMS ワンタイムコード(OTP)のみ**(BeReal と同方式)。パスワードは持たない
  - 選定理由:①1 人 1 アカウントの実在性が担保され、複垢による不正(ストリーク偽装、ブロック回避の作り直し)を構造的に抑止 ②パスワードレスで登録・復帰の摩擦が最小 ③連絡先同期による友達推薦(v1.0)と自然に接続
  - Apple 審査の補足:Sign in with Apple の必須要件(ガイドライン 4.8)は「サードパーティのソーシャルログインを提供する場合」に適用されるため、**電話番号認証のみなら Apple サインインは不要**(BeReal が実例)
  - SMS 基盤:Twilio Verify(国際到達率・OTP 特化の不正検知)。SMS ポンピング詐欺対策として IP/端末ごとの発行レート制限+疑わしい国番号のブロックリストを初日から有効化
  - 機種変更:同じ電話番号への OTP で記録を引き継ぎ。番号変更はアプリ内の「番号引き継ぎ」(旧番号 or 復旧コードで本人確認)で対応。解約番号の再割当による乗っ取りリスクは、180 日以上未ログインのアカウントへの新規 OTP ログイン時に追加確認(生年月日+ユーザー名)で緩和
...
- スキップ宣言はカレンダー上「休み」として達成とも未達成とも区別して表示。**ストリークは加算されないが途切れもしない**(**2 週間に 1 回まで**。無制限だとストリークが形骸化するため。未使用分の翌期間への繰り越しはなし)
...
**猶予時間の設計:デフォルト 3 分(緊迫感を最優先)**

| 案 | メリット | デメリット | 判定 |
|---|---|---|---|
| 2 分(BeReal 同等) | 緊張感最大 | 寝起きの認知能力では操作ミスのリカバリ余地がなく、理不尽な失敗が増える | ✕ |
| **3 分** | **「通知を見た瞬間に体を起こして撮る」以外の行動を許さない緊迫感。二度寝の入り込む余地がゼロ**。起床の証明としての純度が最も高い | 達成率は 15 分案より下がる(挫折→離脱リスク)。トイレ等の割り込みで失敗しうる | **◎ デフォルト採用** |
| 5〜10 分 | 現実的な割り込みに耐えられ達成率が上がる | 緊張感が薄れ、「あと少し目を閉じる」余地が生まれる | △(緩和オプションとして提供) |
| 15 分以上 | 達成率は最も上がる | 一度起きて微睡んでも間に合うため、起床の証明として弱い | ✕ |

- 猶予は **3 分(デフォルト)/ 5 分 / 10 分**から選択可。短いほどボーナスポイント係数が高い(§2.7)— 緊迫感を自分で選び、選んだ緊張に報いる設計
- 3 分のリスク(挫折→離脱)は「遅刻達成」の救済とストリーク復帰導線(§2.6)で吸収し、βで達成率とリテンションを実測して最終判断する
- 通知は起床時刻に 1 回+未反応なら 1 分後・2 分後にリマインド(計 3 回)
...
| 10 日 | ★★ | 「二桁いっちゃった人」 | 同上+プロフィールにバッジ常設。キャラ:布団を蹴破ってキメ顔の人 |
| 14 / 21 日 | ★★ | 「朝コーヒーの常連さん」「それもう習慣の人」 | キャラ:マグ片手に余裕の半笑い/歯磨きしながら涼しい顔。習慣形成の科学(21 日説)に触れる称賛メッセージ |
| 30 日 | ★★★ | 「皆勤のカリスマ」 | 最大演出:日の出ムービー+30 日分の写真がモザイクアートに合成される特別カード(保存・共有可)。キャラ:サングラスでキメてるのに寝癖が残っている人 |
| 50 / 100 日 | ★★★ | 「朝のぬし」「めざめのレジェンド」 | キャラ:ラジオ体操最前列の貫禄の主/金色オーラをまとう伝説の人。100 日でアプリアイコン変更権(内発的報酬の最高峰)を解放 |
| 以降 50 日毎 | ★★★ | レジェンドの進化(150 日=金、200 日=虹のオーラ/衣装変化)+日数刻印 | キャラの衣装・オーラが進化し続ける。演出はポップさを保ちつつ、文言は簡潔に |

**称号のトーン設計:「1 人の人間が成長していく物語」方式。** マイルストーンごとに別々のキャラクターを出すのではなく、**同じ 1 人の人物(ユーザーの分身となる主人公)が、節目を越えるたびに成長した姿に進化していく**。パジャマ姿で布団と格闘していた主人公(3 日)が、朝焼けを走り(5 日)、コーヒーの似合う常連になり(14 日)、カリスマの風格をまとい(30 日)、最後は金色オーラの伝説(100 日)へ——称号はその成長段階の名前。マイルストーン到達時には**進化アニメーション**(旧い姿が光に包まれ、新しい服装・髪型・オーラの姿へ変身)を流し、プロフィールには現在の成長段階の姿が常時表示される。**ストリークが途切れると主人公はパジャマ姿に戻る**(詳細は「途切れた日の設計」)——現在の姿は「いま継続している自分」の鏡であり、これが継続の最大の動機になる。「自分の分身が育っていく」ことで、ストリーク継続がキャラ育成の感覚になる。歴代の姿はバッジ・演出・スタンプ絵柄として図鑑的に振り返れる。

**キャラクターのアートディレクション(「悪口診断」風):**
- ビジュアル:**アニメ調のデフォルメ全身キャラ(2〜3 頭身)。太くクリーンな輪郭線、鮮やかなフラット配色、背景から切り抜けるステッカー風**。ラフな落書き系ではなく、しっかり描き込まれたイラストで 1 枚絵として成立させる
- キャラクターは**動物ではなく人**。同一人物の成長段階を、服装・小物・ポーズの変化で一発で伝える(パジャマ+グローブ → ジャージでドヤ顔 → マグカップ片手の余裕 → サングラス+寝癖 → 金色オーラ)。顔立ちは全段階で同一人物とわかるよう固定
- 進化演出:マイルストーン到達時に現在の姿が光に包まれ、次の姿へ変身するアニメーション(ゲームの進化演出のワクワク感)。変身後の 1 枚絵がそのまま達成カードになる
- 表情に性格を乗せる:ドヤ顔・半目・不敵な笑みなど、**ちょっと自意識のあるコミカルな表情**が「悪口診断」テイストの核。無個性なスマイルにしない。寝癖・あくびなど「本人も朝はつらい」ディテールで共感を誘う
- 文言との組み合わせ:キャラは飄々としているのに文言はちゃんと褒めてくる、というギャップを狙う(例:半目でコーヒーをすする常連さん+「二桁とか、正直すごいと思ってる。」)。※悪口診断と違い**毒はユーザーに向けない**(「失敗に優しい」の設計原則を厳守。ツッコミの矛先は常にキャラ自身や"布団"などに向ける)
- 達成カード:診断結果画像のような**キャッチコピー付きポスター風レイアウト**(キャラ大きめ+称号名+日数+一言)で生成し、SNS や友達フィードにそのままシェアしたくなる画角にする

- マイルストーン間の日も、投稿完了時に必ず 1 行の称賛メッセージ+ポイント加算演出を出す(無音の日を作らない)
- **途切れた日の設計:主人公はパジャマ姿に戻る。** ストリークが途切れると、キャラクターは初期のパジャマ姿にリセットされる——鏡の前でパジャマに戻った自分を見る悔しさが、再起の燃料になる(育成ゲームで進化前に戻る感覚)。ただし演出とメッセージは徹底してポジティブに保つ:
  - リセット時の見せ方:責める画面は出さない。翌朝アプリを開いたときに、パジャマ姿の主人公が布団の上で仕切り直しのストレッチをしている 1 枚絵+「また育てよう。前は 21 日までいけた。」
  - **獲得済みの称号・歴代の姿は図鑑に永久保存**(はく奪しない)。最長記録もプロフィールに残る。失うのは「現在の姿」だけ
  - 再挑戦時はマイルストーンを最初から踏み直す(3 日→5 日→…)。一度見た進化でも「取り戻していく」体験として機能する
  - 翌日の通知文言は「また今日から始めよう。最長記録 N 日はあなたの実力です」。復帰初回達成に「おかえりボーナス」ポイントを付与(途切れの罰ではなく再開の祝福)
  - なお「おやすみ(スキップ、2 週間に 1 回)」を使った日は途切れ扱いにならず、姿もそのまま維持される——スキップ制度が「パジャマに戻りたくない」人の正規の逃げ道として機能する
...
- **ユーザーとして**、貯めたポイントでレベルや演出の変化を楽しみたい。**なぜなら**、数字と見た目の成長が続ける理由になるから。
...
| 起床達成 | +20 | 猶予 10 分設定は×1.0、5 分は×1.2、**3 分(デフォルト)は×1.5**(緊迫感の自己選択に報いる) |
...
- **称号はレベルには紐づけない。称号は連続日数(ストリーク)のみで獲得**(§2.6)——「称号=継続の証」という意味を薄めないため、レベルは数字とゲージ表示のみのシンプルな設計とする
...
#### カメラ
- 画面上部:残り時間のカウントダウンリング。開始時は白 → 残り 1 分で琥珀色 → **残り 30 秒で赤に変化し、数字が脈動+1 秒ごとのハプティクスで緊迫感を最大化**(ラスト 10 秒は秒表示が拡大)
...
- 猶予時間中はヘッダーが「⏰ 残り 2:14 — 今すぐ撮る」の CTA バナーに変化(タップでカメラ直行。残り 30 秒からはバナーも赤に同期)
...
3. **起床時刻**:Scheduler → SQS → Lambda ワーカーが FCM/APNs へ送信。同時に「猶予締切ジョブ」(3 分後)も登録し、締切時に未投稿なら未達成を確定+ストリークリセット処理
...
erDiagram
    ...
    USERS {
        ...
        string phone_number "E.164形式・一意・ハッシュ化検索用列を別途保持"
        date birth_date "未成年判定"
        string timezone "IANA形式"
        int level
        bigint total_points
        string title "現在の称号(ストリーク由来のみ)"
    ...
    STREAKS {
        ...
        date last_skip_date "おやすみは2週間に1回"
        timestamptz updated_at
    }
...
- 電話番号は認証の唯一のキーとなる重要 PII として扱う:保存時は KMS で列レベル暗号化、検索はハッシュ化列で実施、ログ・分析基盤へは一切流さない。連絡先同期(v1.0)ではアップロードされた友人の番号をハッシュ照合のみに使い、平文保存しない
...
**スコープ:** §1.3 の「含める」列すべて。
認証、起床時刻設定+前日確認通知、起床通知+猶予 3 分、アプリ内カメラ投稿、カレンダー、ストリーク+マイルストーン演出(3/5/7/10/30 日)、ログインボーナス+レベル、友達フィード+リアクション、友達管理、通報、不正防止の基本(トークン+サーバー時刻+ロール禁止)。
...
**MVP の検証仮説:** ①前日確認通知は応答され、応答者の達成率は有意に高いか ②猶予 3 分で達成率 50%を超えるか(下回る場合は 5 分へのデフォルト変更を検討) ③友達有無で D30 リテンションに差が出るか
